...
class ReportsViewModel : ViewModel() {
    ...
    fun fetchFullyClearedStudents(gradeLevelId: Int? = null, sectionId: Int? = null) {
        viewModelScope.launch {
            ...
            _error.value = null
            try {
                // This URL was already correct assuming the router is mounted at /reports
    ...
    private fun fetchGradeLevels() {
        viewModelScope.launch {
            try {
                // ✅ FIXED: Added "/reports" prefix
                val response: List<GradeLevelItem> = client.get("/reports/grade-levels").body()
                _gradeLevels.value = response
            } catch (e: Exception) {
                // Enhanced error logging to see the actual URL failure in logs
                e.printStackTrace()
    ...
    fun fetchSectionsForGradeLevel(gradeLevelId: Int) {
        viewModelScope.launch {
            try {
                // ✅ FIXED: Added "/reports" prefix
                val response: List<ClassSection> = client.get("/reports/sections/grade-level/$gradeLevelId").body()
